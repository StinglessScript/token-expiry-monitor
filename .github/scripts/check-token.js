...
async function checkTokenExpiry() {
  try {
    console.log(`🔍 Checking token status at: ${new Date().toISOString()}`);
    ...
    console.log(`✅ Token is WORKING normally`);
    
    await sendNotification('INFO', `✅ Token đang hoạt động bình thường. Kiểm tra lúc: ${new Date().toLocaleString('vi-VN')}`, {
      token_url: TOKEN_URL,
      status: 'working',
      access_token_preview: `${accessToken.substring(0, 8)}...`,
      party_id: tokenData.partyId,
    ...
    if (error.response) {
      ...
      if (status === 422 || status === 401 || status === 403) {
        message = `🚨 TOKEN ĐÃ HẾT HẠN hoặc KHÔNG HỢP LỆ! API trả về HTTP ${status}`;
      } else if (status >= 500) {
        alertLevel = 'WARNING';
        message = `⚠️ Lỗi server (HTTP ${status}) - có thể là vấn đề tạm thời`;
      } else {
        message = `🚨 Token có vấn đề! API trả về HTTP ${status}`;
      }
    } else if (error.code === 'ENOTFOUND' || error.code === 'ECONNREFUSED') {
      alertLevel = 'WARNING';
      message = `⚠️ Không thể kết nối đến server - kiểm tra network`;
    } else if (error.code === 'ETIMEDOUT') {
      alertLevel = 'WARNING';
      message = `⚠️ Timeout khi gọi API - server phản hồi chậm`;
    } else {
      message = `🚨 Lỗi không xác định khi kiểm tra token: ${error.message}`;
    }
